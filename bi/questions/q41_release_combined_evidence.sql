...
/*__EXPERIMENT_ROWS_END__*/
 ...
 SELECT rm.release_event_id,
  count(DISTINCT ps.collected_at::date) FILTER(WHERE ps.collected_at::date BETWEEN rm.baseline_start AND rm.baseline_end) store_before_days,
  count(DISTINCT ps.collected_at::date) FILTER(WHERE ps.collected_at::date BETWEEN rm.followup_start AND rm.followup_end) store_after_days,
  max(ps.collected_at) store_latest_source_at
 FROM release_manifest rm JOIN public.applications a ON a.package_name=rm.application_package
 LEFT JOIN public.playstore_app_snapshots ps ON ps.application_id=a.id
 GROUP BY rm.release_event_id
), reviews AS (
 SELECT rm.release_event_id,
  count(DISTINCT r.id) FILTER(WHERE (r.first_observed_at AT TIME ZONE 'UTC')::date
    BETWEEN rm.baseline_start AND rm.baseline_end) review_before_n,
  count(DISTINCT r.id) FILTER(WHERE (r.first_observed_at AT TIME ZONE 'UTC')::date
    BETWEEN rm.followup_start AND rm.followup_end) review_after_n,
  max(r.first_observed_at) review_latest_source_at
 FROM release_manifest rm JOIN public.applications a ON a.package_name=rm.application_package
 LEFT JOIN public.reviews r ON r.application_id=a.id GROUP BY rm.release_event_id
...
SELECT rm.release_event_id,rm.application_package,rm.previous_version,rm.new_version,rm.release_date,
 rm.release_date_precision,rm.release_date_verified,n.scenario,n.file_cohort_id,n.test_file_size_bytes,
 ...
 CASE WHEN NOT rm.release_date_verified OR rm.release_date_precision<>'day' THEN 'release_metadata_insufficient'
      ...
      WHEN s.store_before_days=0 OR s.store_after_days=0 THEN 'store_period_missing'
      WHEN r.review_before_n=0 OR r.review_after_n=0 THEN 'review_cohort_missing'
      WHEN c.network_state NOT IN('NETWORK_DATA_AVAILABLE','NETWORK_COMPARISON_INSUFFICIENT','NETWORK_COMPARISON_READY')
       THEN 'network_unavailable'
 ...
 'Readiness is per application, release, scenario and matched file/device/Android/network/tool conditions, with sources remaining observational and separate'::text limitation
FROM release_manifest rm LEFT JOIN store s USING(release_event_id)
LEFT JOIN reviews r USING(release_event_id) LEFT JOIN network n USING(release_event_id)
...
ORDER BY rm.release_date,rm.application_package,n.scenario,n.file_cohort_id,n.device_model,n.network_profile;
