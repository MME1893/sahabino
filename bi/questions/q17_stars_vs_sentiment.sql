...
-- Different measures; unclassified reviews show as status rather than synthesized sentiment.
SELECT application_name,package_name,crawl_country_code,crawl_language_code,
 score AS review_stars_1_to_5,
 CASE WHEN sentiment_status='done' AND sentiment_label IN('positive','neutral','negative')
 THEN sentiment_label ELSE 'unclassified' END AS sentiment_measure,
 COUNT(*) AS unique_review_count,
 COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label IN('positive','neutral','negative')) AS classified_sample_size
FROM cohort GROUP BY application_name,package_name,crawl_country_code,crawl_language_code,score,
 CASE WHEN sentiment_status='done' AND sentiment_label IN('positive','neutral','negative') THEN sentiment_label ELSE 'unclassified' END
ORDER BY application_name,score,sentiment_measure;
