...
-- Most recent eligible observation per unique review, as-of end date. Skipped remain missing.
SELECT application_name,package_name,crawl_country_code,crawl_language_code,
 ...
 COUNT(*) FILTER(WHERE sentiment_status='skipped') AS skipped_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='failed') AS failed_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='done') AS done_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label IN('positive','neutral','negative')) AS classified_unique_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label IN('positive','neutral','negative'))::numeric/NULLIF(COUNT(*),0) AS classification_coverage,
