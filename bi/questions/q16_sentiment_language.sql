...
-- sentiment_language is inferred classification language, distinct from crawl locale.
SELECT application_name,package_name,crawl_country_code,crawl_language_code,sentiment_language,sentiment_label,
 COUNT(*) AS unique_classified_reviews,
 SUM(COUNT(*)) OVER(PARTITION BY application_name,package_name,crawl_country_code,crawl_language_code,sentiment_language) AS language_classified_sample_size,
 COUNT(*)::numeric/NULLIF(SUM(COUNT(*)) OVER(PARTITION BY application_name,package_name,crawl_country_code,crawl_language_code,sentiment_language),0) AS share_in_language
FROM cohort WHERE sentiment_status='done' AND sentiment_label IN('positive','neutral','negative')
GROUP BY application_name,package_name,crawl_country_code,crawl_language_code,sentiment_language,sentiment_label
ORDER BY application_name,sentiment_language,sentiment_label;
