...
-- Latest-known unique review at cutoff; bucket by FIRST encounter, not inference event.
SELECT application_name,package_name,crawl_country_code,crawl_language_code,
 ...
 COUNT(*) AS eligible_unique_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label IN('positive','neutral','negative')) AS classified_unique_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label='negative') AS negative_classified_reviews,
 COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label='negative')::numeric/
 NULLIF(COUNT(*) FILTER(WHERE sentiment_status='done' AND sentiment_label IN('positive','neutral','negative')),0) AS negative_sentiment_share
FROM cohort GROUP BY application_name,package_name,crawl_country_code,crawl_language_code,(first_observed_at AT TIME ZONE 'UTC')::date
