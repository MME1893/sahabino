...
Q41's base Release evidence does not require the optional Network schema. When Network schema or column grants are unavailable, compilation substitutes a typed empty Network relation and persists no Network table references in the Saved Question. Store observations are grouped by the historical `crawl_tasks.country_code` and `language_code`; readiness requires both release periods within one such locale. Review observations use their own historical crawl locale and UTC period cutoff and join only to the identical selected Store locale, so neither Store nor Review evidence crosses locales.
